...
describe('OpenAPI generator examples', () => {
  it('generates schema examples and response examples when --examples is true', async () => {
    ...
    const { Theme } = require('../src/constants');
    await generateOpenAPIDocument(sequelize as any, '/tmp/openapi-test.json', true, Theme.Names);
  ...
  it('generateExampleForAttribute returns proper types', () => {
    ...
    expect(generateExampleForAttribute('birthdate', { type: 'DATE' }, Theme.Lorem)).toMatch(/\d{4}-\d{2}-\d{2}/);
  });
});
